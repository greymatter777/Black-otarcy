...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (user.audits_limit !== -1 && user.audits_used >= user.audits_limit) return res.status(403).json({ error: "Limite d'audits atteinte." });
...
Réponds UNIQUEMENT en JSON valide, sans texte avant ou après :
{
  ...
  try {
    ...
    await supabase.from("audits").insert({
      user_id: clerkUserId, brand,
      score: parsed.score ?? 0, analysis: parsed.analysis ?? "",
      strengths: parsed.strengths ?? [], weaknesses: parsed.weaknesses ?? [],
      recommendations: parsed.recommendations ?? [],
      swot: parsed.swot ?? null, kpis: parsed.kpis ?? null,
    });

    await supabase.from("users").update({ audits_used: user.audits_used + 1, updated_at: new Date().toISOString() }).eq("id", clerkUserId);

    return res.status(200).json({
      score: parsed.score ?? 0, analysis: parsed.analysis ?? "",
      strengths: parsed.strengths ?? [], weaknesses: parsed.weaknesses ?? [],
      recommendations: parsed.recommendations ?? [],
      swot: parsed.swot ?? null, kpis: parsed.kpis ?? null, plan: user.plan,
    });
  } catch (err: any) {
